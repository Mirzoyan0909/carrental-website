...
import { Phone, MessageCircle, Car, Menu, X, Plus, Edit, Trash2, LogOut, Settings, Fuel, Gauge, Calendar, ShieldAlert, Shield, Upload, Link as LinkIcon, Zap, Award, Users, Clock } from "lucide-react";
    ...
    <header className={`bg-[#050505] text-white border-b border-white/10 sticky top-0 z-50 transition-all duration-500 ${scrolled ? 'shadow-2xl backdrop-blur-xl bg-[#050505]/95' : ''}`}>
      <div className="container-custom flex items-center justify-between h-20">
        ...
        <nav className="hidden md:flex items-center gap-10">
          <button onClick={() => scrollTo('catalog')} className="nav-link">Каталог</button>
          <button onClick={() => scrollTo('about')} className="nav-link">О Нас</button>
          <button onClick={() => scrollTo('contact')} className="nav-link">Контакты</button>
        </nav>
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#050505] border-t border-white/10 py-6 animate-fadeIn">
          <nav className="flex flex-col items-center gap-6">
            <button onClick={() => scrollTo('catalog')} className="nav-link text-base">Каталог</button>
            <button onClick={() => scrollTo('about')} className="nav-link text-base">О Нас</button>
            <button onClick={() => scrollTo('contact')} className="nav-link text-base">Контакты</button>
          </nav>
        </div>
      )}
    </header>
...
const HeroSection = () => (
  <section className="hero-section" data-testid="hero-section">
    ...
    <div className="hero-content">
      <p className="text-[#D4AF37] uppercase tracking-[0.3em] text-sm md:text-base mb-6 font-semibold">Премиум Автомобили</p>
      <h1 className="hero-title font-heading font-black mb-8">Продажа & Аренда</h1>
      <p className="hero-subtitle max-w-3xl mx-auto mb-12">Эксклюзивная коллекция премиальных автомобилей для самых взыскательных клиентов</p>
  ...
  <div className="car-card" data-testid={`car-card-${car.id}`}>
    <div className="aspect-[4/3] overflow-hidden bg-gray-100 relative">
      ...
      <div className="absolute top-4 left-4 flex gap-2">
        {car.for_sale && <span className="badge-sale text-xs px-4 py-2 uppercase tracking-wider font-semibold rounded-sm">Продажа</span>}
        {car.for_rent && <span className="badge-rent text-xs px-4 py-2 uppercase tracking-wider font-bold rounded-sm">Аренда</span>}
      </div>
    </div>
    <div className="p-6 space-y-4">
      ...
      <div className="flex flex-wrap gap-4 text-sm text-gray-600">
        ...
        <span className="flex items-center gap-2"><Gauge className="w-4 h-4" />{car.mileage?.toLocaleString()} км</span>
      ...
      <div className="pt-4 border-t-2 border-gray-100 space-y-3">
        {car.for_sale && car.price_sale && (
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onContact(car, 'sale')} className="btn-dark px-8 py-3 text-xs uppercase font-bold rounded-sm hover:scale-105" data-testid={`buy-${car.id}`}>Купить</button>
          </div>
        )}
        {car.for_rent && car.price_rent && (
          <div className="flex items-center justify-between">
            <span className="price-tag text-2xl">${car.price_rent.toLocaleString()}/день</span>
            <button onClick={() => onContact(car, 'rent')} className="btn-gold px-8 py-3 text-xs uppercase font-bold rounded-sm hover:scale-105" data-testid={`rent-${car.id}`}>Арендовать</button>
          </div>
        )}
      </div>
    </div>
...
const ContactModal = ({ open, onClose, car, type, contact }) => (
  <Dialog open={open} onOpenChange={onClose}>
    <div className="bg-gradient-to-r from-[#050505] to-[#1a1a1a] text-white p-10">
      <h2 className="font-heading text-3xl font-bold mb-2">{type === 'rent' ? 'Аренда' : 'Покупка'} {car?.title}</h2>
      <p className="text-white/80 text-lg">Свяжитесь с нами для {type === 'rent' ? 'аренды' : 'покупки'} этого автомобиля</p>
    </div>
    <div className="p-10 space-y-6">
      <a href={`tel:${contact?.phone || '+1 (555) 123-4567'}`} className="contact-link">
        ...
        <div>
          <p className="text-xs uppercase text-gray-400 mb-1 font-semibold tracking-wider">Позвонить</p>
      ...
      <a href={`https://t.me/${(contact?.telegram || '@premium_auto').replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="contact-link">
        <div className="w-14 h-14 bg-gradient-to-br from-[#D4AF37] to-[#C5A028] flex items-center justify-center rounded-lg shadow-lg">
    ...
    <section id="catalog" className="section-padding bg-gradient-to-b from-gray-50 to-white" data-testid="catalog-section">
      <div className="container-custom">
        <div className="text-center mb-20 section-title">
          <p className="text-[#D4AF37] uppercase tracking-[0.3em] text-sm mb-4 font-bold">Наш Инвентарь</p>
          <h2 className="font-heading text-4xl md:text-6xl font-bold text-gray-900 mb-4">Автомобили</h2>
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">Премиальная коллекция для вас</p>
        </div>
        <div className="flex justify-center mb-16">
          <div className="inline-flex bg-white border-2 border-gray-200 shadow-xl rounded-sm overflow-hidden">
            <button className={`filter-tab ${filter === 'all' ? 'active' : ''}`} onClick={() => setFilter('all')} data-testid="filter-all">Все</button>
            <button className={`filter-tab ${filter === 'sale' ? 'active' : ''}`} onClick={() => setFilter('sale')} data-testid="filter-sale">Продажа</button>
            <button className={`filter-tab ${filter === 'rent' ? 'active' : ''}`} onClick={() => setFilter('rent')} data-testid="filter-rent">Аренда</button>
          </div>
        </div>
        {filteredCars.length === 0 ? (
          <div className="text-center py-24">
            <Car className="w-20 h-20 text-gray-300 mx-auto mb-4" />
            <p className="text-gray-400 text-xl">Автомобили не найдены</p>
          </div>
        ) : (
    ...
    <div className="container-custom">
      <div className="grid md:grid-cols-2 gap-20 items-center">
        <div className="section-title">
          <p className="text-[#D4AF37] uppercase tracking-[0.3em] text-sm mb-4 font-bold">О Нас</p>
          <h2 className="font-heading text-4xl md:text-6xl font-bold text-gray-900 mb-8 leading-tight">Премиум Качество & Сервис</h2>
          <p className="text-gray-600 mb-6 text-lg leading-relaxed">Мы предлагаем эксклюзивную коллекцию премиальных автомобилей для продажи и аренды. Каждый автомобиль тщательно отобран и проверен.</p>
...
const ContactSection = ({ contact }) => (
  <section id="contact" className="section-padding bg-gradient-to-br from-[#050505] via-[#0a0a0a] to-[#050505] text-white" data-testid="contact-section">
    <div className="container-custom">
      <div className="text-center mb-20 section-title">
        <p className="text-[#D4AF37] uppercase tracking-[0.3em] text-sm mb-4 font-bold">Свяжитесь с Нами</p>
        <h2 className="font-heading text-4xl md:text-6xl font-bold mb-4">Контакты</h2>
        <p className="text-white/70 text-lg max-w-2xl mx-auto">Мы всегда рады вам помочь</p>
      </div>
      <div className="max-w-3xl mx-auto grid md:grid-cols-2 gap-8">
        ...
        >
          ...
          <p className="text-xs uppercase text-white/50 mb-2 font-semibold tracking-wider">Телефон</p>
        ...
        >
          <MessageCircle className="w-12 h-12 text-[#D4AF37] mb-6 group-hover:scale-110 transition-transform" />
...
const Footer = () => (
  <footer className="bg-[#050505] text-white py-16 border-t border-white/10">
    <div className="container-custom flex flex-col md:flex-row items-center justify-between gap-6">
      ...
      <p className="text-white/50 text-sm">© {new Date().getFullYear()} Premium Auto. Все права защищены.</p>
    </div>
  </footer>
);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 p-4">
      <div className="w-full max-w-md animate-fadeInUp">
        <div className="bg-gradient-to-r from-[#050505] to-[#1a1a1a] p-10 text-center shadow-2xl">
          ...
          <h1 className="font-heading text-3xl font-bold text-white mb-2">Админ Панель</h1>
          <p className="text-white/70">Вход в систему управления</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-white p-10 shadow-2xl space-y-6">
          ...
          <div>
            <Label className="text-xs uppercase text-gray-600 mb-2 block">Логин</Label>
            <Input 
              value={username} 
          ...
          <div>
            <Label className="text-xs uppercase text-gray-600 mb-2 block">Пароль</Label>
            <Input 
              type="password" 
              value={password} 
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <header className="bg-gradient-to-r from-[#050505] to-[#1a1a1a] text-white h-20 flex items-center px-8 sticky top-0 z-50 shadow-2xl">
        <div className="flex items-center gap-3">
          ...
          <span className="font-heading text-xl font-bold">Админ Панель</span>
        </div>
      ...
      <div className="p-8 md:p-12">
        <div className="bg-white border-2 border-gray-200 p-8 mb-10 shadow-xl rounded-lg">
          <div className="flex items-center justify-between mb-8">
            <h2 className="font-heading text-2xl font-bold text-gray-900">Контактная Информация</h2>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="p-6 bg-gradient-to-br from-gray-50 to-white rounded-lg border border-gray-200">
              <p className="text-xs uppercase text-gray-500 mb-2 font-semibold tracking-wider">Телефон</p>
        ...
        <div className="bg-white border-2 border-gray-200 p-8 shadow-xl rounded-lg">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="font-heading text-2xl font-bold text-gray-900">Автомобили</h2>
              <p className="text-gray-600 mt-1">Всего: {cars.length}</p>
            </div>
            <Button 
              ...
              <tbody>
                {cars.map(car => (
                  <tr key={car.id} data-testid={`admin-car-row-${car.id}`}>
                    ...
                    <td className="font-semibold text-[#D4AF37]">{car.for_rent ? `$${car.price_rent}/день` : '—'}</td>
                    <td>
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <div className="bg-gradient-to-r from-[#050505] to-[#1a1a1a] text-white p-8">
        <h2 className="font-heading text-2xl font-bold">{car ? 'Редактировать Автомобиль' : 'Добавить Автомобиль'}</h2>
      </div>
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <Label className="mb-2 block">Коробка</Label>
        ...
        <div>
          <Label className="mb-3 block">Фото Автомобиля*</Label>
          ...
          {uploadMode === 'url' ? (
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-[#D4AF37] transition-all">
              ...
              <label htmlFor="file-upload" className="cursor-pointer">
                ...
                <p className="text-sm text-gray-400">PNG, JPG, JPEG до 10MB</p>
              </label>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-8 pt-6 border-t-2 border-gray-200">
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <Label className="font-bold">Доступно для Продажи</Label>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <Label className="font-bold">Доступно для Аренды</Label>
              <Switch checked={form.for_rent} onCheckedChange={(v) => setForm({...form, for_rent: v})} />
            </div>
            {form.for_rent && (
              <div>
                <Label className="mb-2 block">Цена ($/день)</Label>
                <Input 
                  type="number" 
...
const ContactFormModal = ({ open, onClose, contact, onSave }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <div className="bg-gradient-to-r from-[#050505] to-[#1a1a1a] text-white p-8">
        <h2 className="font-heading text-2xl font-bold">Контактная Информация</h2>
      </div>
